...
The form is split into horizontal tabs (General / Storage / Library / Notifications /
Advanced) so it reads as five short screens instead of one long scroll. Every field
across all tabs is written by the single ``save()`` closure, surfaced through a sticky
save bar at the bottom that stays reachable from any tab. The action buttons (scan,
backfill, notification test, exports) operate on *saved* settings — save first.
"""
...
def _section(key: str) -> None:
    """A small uppercase section label used to group fields inside a tab."""
    ui.label(t(key)).classes("text-xs uppercase tracking-widest text-white/45 pt-1")
...
def settings_content() -> None:
    ...
    # Page heading: icon + title + subtitle (mirrors the download page).
    with ui.row().classes("items-center gap-3"):
        ui.icon("settings", size="28px").classes("accent-text")
        ui.label(t("settings.title")).classes("text-3xl font-bold text-white")
    ui.label(t("settings.subtitle")).classes("text-white/50 text-sm")

    # Everything lives in one glass card: a tab bar, the panels, and a sticky save bar.
    with ui.card().classes("glass w-full rounded-2xl p-0 gap-0 overflow-hidden"):
        with ui.tabs().props("no-caps inline-label active-color=white indicator-color=primary "
                             "align=left").classes("w-full px-2") as tabs:
            tab_general = ui.tab("general", label=t("settings.tab_general"), icon="tune")
            tab_storage = ui.tab("storage", label=t("settings.tab_storage"), icon="cloud")
            tab_library = ui.tab("library", label=t("settings.tab_library"),
                                 icon="library_music")
            tab_notify = ui.tab("notify", label=t("settings.tab_notify"),
                                icon="notifications")
            tab_advanced = ui.tab("advanced", label=t("settings.tab_advanced"),
                                  icon="build")

        with ui.tab_panels(tabs, value=tab_general).classes("w-full bg-transparent"):
            # ---- General: defaults + metadata --------------------------------------------
            with ui.tab_panel(tab_general).classes("flex flex-col gap-4"):
                _section("settings.section_defaults")
                ...
                _section("settings.section_metadata")
            ...
            # ---- Storage: WebDAV target + dedup ------------------------------------------
            with ui.tab_panel(tab_storage).classes("flex flex-col gap-3"):
                ...
                wd_url = ui.input(
                    t("settings.webdav_url_label"), value=snap["wd_url"],
                ...
                wd_pass = ui.input(t("settings.password"), password=True,
                    ...
                    with dialog, ui.card().classes("glass w-full max-w-md rounded-2xl p-4 gap-2"):
                        ...
                        async def load() -> None:
                            ...
                            try:
                                ...
                                with lst:
                                    ui.label(t("settings.picker_error", error=exc)) \
                                        .classes("text-red-400 text-sm")
                                return
                            with lst:
                                if cur["path"]:
                                    ghost_button(t("settings.picker_back"), on_click=go_up) \
                                        .props("dense")
                                if not dirs:
                                    ui.label(t("settings.picker_no_sub")) \
                                        .classes("text-white/40 text-sm")
                                for name, full in dirs:
                                    ghost_button(f"📁 {name}", on_click=lambda f=full: enter(f)) \
                                        .props("dense align=left").classes("w-full")

                        def choose() -> None:
                            folder_state["path"] = cur["path"]
                            folder_lbl.text = _folder_text()
                            dialog.close()
                            ui.notify(t("settings.folder_chosen", path=cur["path"]),
                                      type="positive")

                        with ui.row().classes("w-full justify-end gap-2 pt-2"):
                            ghost_button(t("settings.cancel"), on_click=dialog.close)
                            primary_button(t("settings.picker_choose"), icon="check",
                                           on_click=choose)
                    await load()
                    dialog.open()
                ...
                with ui.row().classes("items-center gap-3 flex-wrap"):
                    primary_button(t("settings.connect_button"), icon="folder_open",
                                   on_click=browse)
                    folder_lbl = ui.label(_folder_text()).classes("text-sm text-white/70")

                # Dedup (issue #31): WebDAV-only — skip already-present tracks; reference them
                # in playlist m3u. Lives with the storage target it applies to.
                _section("settings.section_defaults")
                ui.label(t("settings.dedup_desc")).classes("text-xs text-white/50")
                dedup_sw = ui.switch(t("settings.dedup_label"), value=snap["dedup"]) \
                    .props("dark").classes("text-sm")

            # ---- Library: maintenance ----------------------------------------------------
            with ui.tab_panel(tab_library).classes("flex flex-col gap-3"):
                _section("settings.section_maintenance")
                ...
                def _scan_done(result) -> tuple[str, str]:
                    ...
                    if errors:
                        return "warning", t("settings.scan_incomplete", count=added,
                                            failed=len(errors))
                ...
                async def scan_server() -> None:
                    ...
                    def _scan():
                        # Guarded run: skips if a scheduled/other scan is already walking this
                        # library.
                        result = jobs.run_scan_sync(uid)
                        if result is None:
                            return None
                        # Opportunistic trash purge (roadmap 01): expired trash folders are
                        # cleaned up here so no separate scheduler is needed. Best-effort —
                ...
                secondary_button(t("settings.scan_button"), icon="cloud_sync",
                                 on_click=scan_server).classes("self-start")
                ...
                def _backfill_done(result) -> tuple[str, str]:
                    ...
                    if errors:
                        return "warning", t("settings.lyrics_backfill_incomplete",
                                            written=written, failed=len(errors))
                ...
                secondary_button(t("settings.lyrics_backfill_button"), icon="lyrics",
                                 on_click=backfill_lyrics_server).classes("self-start")

                # Trash safety net (roadmap 01): a deleted library file is first moved into a
                # dated trash folder and hard-deleted only after this many days (0 = immediately).
                ...
                # Navidrome deep link (roadmap 03): when set, the library page links each album
                # to a Navidrome search. Optional and API-free — just a base URL.
                ui.label(t("settings.navidrome_hint")).classes("text-xs text-white/50 mt-2")
                navidrome_url_in = ui.input(t("settings.navidrome_url"),
                                            value=snap["navidrome_url"],
                                            placeholder="https://music.example.org") \
                    .props("outlined dense dark").classes("w-full")

                # Optional: browse the trash, restore a file or empty it.
                with ui.expansion(t("settings.trash_title"), icon="delete").classes("w-full"):
                    ...
                    async def refresh_trash() -> None:
                        ...
                        except Exception as exc:  # noqa: BLE001 - surface config/conn errors
                            with trash_list:
                        ...
                        with trash_list:
                            if not entries:
                                ui.label(t("settings.trash_empty_state")) \
                                    .classes("text-white/40 text-sm")
                                return
                            for entry in entries:
                                async def _restore(e=entry) -> None:
                                    from app import library_ops
                                    try:
                                        await run.io_bound(library_ops.restore_track, uid,
                                                           e.trash_rel)
                                    except Exception as exc:  # noqa: BLE001
                                        ui.notify(t("settings.trash_error", error=exc),
                                                  type="negative")
                                        return
                                ...
                                with ui.row().classes(
                                    ...
                                    ghost_button(t("settings.trash_restore"), icon="restore",
                                                 on_click=_restore).props("dense")
                    ...
                    with ui.row().classes("w-full gap-2 pt-1"):
                        ghost_button(t("settings.trash_refresh"), icon="refresh",
                                     on_click=refresh_trash).props("dense")
                        ghost_button(t("settings.trash_empty"), icon="delete_forever",
                                     on_click=empty_trash).props("dense") \
                            .classes(remove="text-white/70", add="text-red-300")

            # ---- Notifications (issue #42) -----------------------------------------------
            with ui.tab_panel(tab_notify).classes("flex flex-col gap-3"):
                ...
                n_new_sw = ui.switch(t("notify.event_new_tracks"), value=snap["n_new_tracks"]) \
                    .props("dark").classes("text-sm")
                n_syncerr_sw = ui.switch(t("notify.event_sync_error"),
                                         value=snap["n_sync_error"]) \
                    .props("dark").classes("text-sm")
                n_dlerr_sw = ui.switch(t("notify.event_download_error"),
                                       value=snap["n_dl_error"]) \
                    .props("dark").classes("text-sm")

                ui.label(t("notify.ntfy_heading")) \
                    .classes("text-sm font-semibold mt-2 text-white/80")
                ...
                ui.label(t("notify.webhook_heading")) \
                    .classes("text-sm font-semibold mt-2 text-white/80")
                n_webhook_url = ui.input(t("notify.webhook_url_label"),
                                         value=snap["n_webhook_url"]) \
                    .props("outlined dense dark").classes("w-full")

                ui.label(t("notify.email_heading")) \
                    .classes("text-sm font-semibold mt-2 text-white/80")
                with ui.row().classes("w-full gap-3 flex-wrap"):
                    ...
                    n_email_from = ui.input(t("notify.email_from_label"),
                                            value=snap["n_email_from"]) \
                        .props("outlined dense dark").classes("flex-1 min-w-40")
                with ui.row().classes("w-full gap-3 flex-wrap items-end"):
                    n_smtp_host = ui.input(t("notify.smtp_host_label"),
                                           value=snap["n_smtp_host"]) \
                        .props("outlined dense dark").classes("flex-1 min-w-40")
                    ...
                    n_security = ui.select({"starttls": t("notify.security_starttls"),
                                           ...
                                           value=snap["n_security"],
                                           label=t("notify.security_label")) \
                ...
                async def send_test_notification() -> None:
                    ...
                    # Persistent spinner we dismiss ourselves — an "ongoing" toast never
                    # auto-closes.
                    note = ui.notification(t("notify.test_running"), type="ongoing",
                ...
                secondary_button(t("notify.test_button"), icon="notifications",
                                 on_click=send_test_notification).classes("self-start mt-2")

            # ---- Advanced: cookie, export/backup, bookmarklet ----------------------------
            with ui.tab_panel(tab_advanced).classes("flex flex-col gap-6"):
                # YouTube cookie (issue #9). The stored cookie is never echoed back — only a
                # "set" state via the placeholder.
                with ui.column().classes("w-full gap-2"):
                    ...
                    cookie_placeholder = t("settings.cookie_placeholder_set") \
                        if snap["has_cookie"] else t("settings.cookie_label")
                    cookie_ta = ui.textarea(t("settings.cookie_label"),
                ...
                # Export & backup (roadmap 17): a user's own data out as downloads + re-import.
                ...
                with ui.column().classes("w-full gap-2"):
                    ...
                    async def _download(fn, kind: str, ext: str, media: str) -> None:
                        ...
                        ui.download.content(data.encode("utf-8"),
                                            f"soundpull-{kind}-{stamp}.{ext}", media)

                    with ui.row().classes("flex-wrap gap-2"):
                        secondary_button(t("settings.export_library_csv"), icon="table_view",
                                         on_click=lambda: _download(exports.library_manifest_csv,
                                                                    "library", "csv", "text/csv"))
                        secondary_button(t("settings.export_library_json"), icon="data_object",
                                         on_click=lambda: _download(exports.library_manifest_json,
                                                                    "library", "json",
                                                                    "application/json"))
                        secondary_button(t("settings.export_history_csv"), icon="history",
                                         on_click=lambda: _download(exports.history_csv, "history",
                                                                    "csv", "text/csv"))
                        secondary_button(t("settings.export_settings_json"), icon="tune",
                                         on_click=lambda: _download(exports.settings_json,
                                                                    "settings", "json",
                                                                    "application/json"))
                    ...
                    def _confirm_import(payload: str) -> None:
                        ...
                        with dialog, ui.card().classes(
                                "glass w-full max-w-md rounded-2xl p-4 gap-2"):
                            ui.label(t("settings.import_confirm_title")).classes("font-semibold")
                            ui.label(t("settings.import_confirm_body", count=len(keys))) \
                                .classes("text-sm text-white/70")
                            if keys:
                                ui.label(", ".join(keys)).classes("text-xs text-white/50 break-all")
                            ui.label(t("settings.import_secrets_note")) \
                                .classes("text-xs text-amber-300 pt-1")
                            ...
                            async def confirm() -> None:
                                ...
                                # Re-render every widget from the freshly-written DB.
                                ui.navigate.reload()

                            with ui.row().classes("w-full justify-end gap-2 pt-2"):
                                ghost_button(t("settings.cancel"), on_click=dialog.close)
                                primary_button(t("settings.import_button"), icon="upload",
                                               on_click=confirm)
                        dialog.open()
                    ...
                    async def _on_upload(e) -> None:
                        ...
                        except Exception as exc:  # noqa: BLE001 - surface a bad/oversized upload
                    ...
                    ui.upload(on_upload=_on_upload, auto_upload=True, max_files=1,
                              max_file_size=1_000_000).props("accept=.json flat dense") \
                        .classes("w-full")

                # Bookmarklet — dragging a javascript: link is blocked in many browsers, so the
                # reliable path is: copy the code, create a bookmark, paste as URL.
                ...
                with ui.column().classes("w-full gap-2"):
                    ...
                    primary_button(t("settings.bm_copy_button"), icon="content_copy",
                                   on_click=copy_bm).classes("self-start")
        ...
        # Sticky save bar: reachable from any tab without hunting for the button.
        with ui.row().classes("w-full items-center justify-between gap-3 px-5 py-3 border-t "
                              "border-white/10").style(
                "position:sticky; bottom:0; background:rgba(16,16,28,0.72); "
                "backdrop-filter:blur(16px); -webkit-backdrop-filter:blur(16px)"):
            ui.label(t("settings.unsaved_hint")).classes("text-xs text-white/45")
            primary_button(t("settings.save_button"), icon="save", on_click=save) \
                .classes("px-6")
